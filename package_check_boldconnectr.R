...
cat("Displaying the dependent packages required by BOLDconnectR that are available with their respective versions and the minimum version required for running the package smoothly. The NA values in the min_version_required either mean that its a core package or the latest version is available on your machine if installed previously:\n\n")
